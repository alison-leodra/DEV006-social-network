...
const onGetPost = (callback) => {
  const q2 = Rl(_h(db, "publish"), Cl("timestamp", "desc"));
  return pf(q2, callback);
};
const deleteDocFirebase = (postId) => {
  const docRef = mh(db, "publish", postId);
  return gf(docRef);
};
const refDocLiked = (postID) => {
  const refDoc = mh(db, "publish", postID);
  return refDoc;
};
const currentUser = () => {
...
const likesArray = async (postDocRef) => {
  const post = await cf(postDocRef);
  const likes = post.data().likes;
  return likes;
};
const incrementLike = (postDocRef, userLike) => {
  mf(postDocRef, {
    likes: Gf(userLike)
  });
};
const decrementLike = (postDocRef, userLike) => {
  mf(postDocRef, {
    likes: Qf(userLike)
  });
};
const updateFirebaseDocument = (postId, postValue) => {
  const docRef = mh(db, "publish", postId);
  return mf(docRef, {
    post: postValue,
    timestamp: Kf()
...
const login = (navigateTo2) => {
  ...
  <div class='imgLogo'>
    ...
    <h2 class='tittle'>Encuentra a tus compa\xF1eros de juego perfectos</h2>
  </div>
</header> `;
  ...
  inputPassword.setAttribute("placeholder", "contrase\xF1a");
  ...
  forgotBtn.innerText = "\xBFOlvidaste tu contrase\xF1a?";
  const registerFooter = document.createElement("footer");
  const imgGeneral = document.createElement("img");
  imgGeneral.setAttribute("src", "./img/Sin t\xEDtulo-4.png");
  ...
  signUpButton.innerText = "\xBFA\xFAn no tienes cuenta? Registrate aqu\xED";
  ...
  logInBtn.addEventListener("click", async (e) => {
    ...
    try {
      ...
      if (error2.code === "auth/invalid-email") {
        ...
        emailError.textContent = "El correo electr\xF3nico ingresado no es v\xE1lido.";
      } else if (error2.code === "auth/user-not-found") {
        emailError.style.display = "block";
        passwordError.style.display = "none";
        emailError.textContent = "El correo electr\xF3nico ingresado no ha sido encontrado";
      } else if (error2.code === "auth/wrong-password") {
        passwordError.style.display = "block";
        emailError.style.display = "none";
        passwordError.textContent = "La contrase\xF1a es incorrecta";
      } else if (error2.code === "auth/missing-password") {
        passwordError.style.display = "block";
        emailError.style.display = "none";
        passwordError.textContent = "Debe ingresar una contrase\xF1a.";
      }
    }
  });
...
const register = (navigateTo2) => {
  const template = `
    <div class="registroOption">
      ...
      <footer>
        <img src="./img/Sin t\xEDtulo-4.png" alt="footer">
      </footer>
    </div>
  `;
  const element = document.createElement("div");
...
const registerEmail = (navigateTo2) => {
  const template = `
    ...
    <main>
      <div class="container">
        <form id="signUpFormEmail">
            ...
            <input type="password" id="signUpPassword" class="formControl" placeholder="contrase\xF1a" required>
  ...
  registerUser.addEventListener("click", async (e) => {
    e.preventDefault();
    const name2 = document.querySelector("#userName").value;
    ...
    try {
      if (name2 === "") {
        ...
        await updateProfile(userCredential.user, { displayName: name2, photoURL: "./img/avatarDefault(1).png" });
        nameError.style.display = "none";
        passwordError.style.display = "none";
        emailError.style.display = "none";
        navigateTo2("/userRegister");
      }
    } catch (error2) {
      if (error2.code === "auth/invalid-email") {
        emailError.style.display = "block";
        ...
        passwordError.style.display = "none";
        nameError.style.display = "none";
        emailError.textContent = "El correo electr\xF3nico ingresado ya est\xE1 en uso.";
      } else if (error2.code === "auth/weak-password") {
        passwordError.style.display = "block";
        emailError.style.display = "none";
        nameError.style.display = "none";
        passwordError.textContent = "La contrase\xF1a debe tener al menos 6 caracteres.";
      } else if (error2.code === "auth/missing-password") {
        passwordError.style.display = "block";
        emailError.style.display = "none";
        nameError.style.display = "none";
...
const userRegister = (navigateTo2) => {
  const template = `
  <div class="userRegister">
    ...
    <main>
      <div class="container">
      <h2 class="tittleUserRegister">\xA1 Registro exitoso !</h2>
      <p>Inicia sesi\xF3n con tu correo y contrase\xF1a.</p>
      <button type="button" class="logInRegister"> Iniciar sesi\xF3n </button>
      </div>
      </main>
    <footer>
...
const home = () => {
  ...
  textarea.setAttribute("placeholder", "Escribe aqu\xED...");
  ...
  onGetPost((querySnapshot) => {
      ...
      html += `
        <div id='modal' class='modal'>
          <div class='modal-content'>
            <h2>\xBFDesea eliminar?</h2>
              <button id='yesBtn'>S\xED</button>
    ...
    yesBtn.addEventListener("click", (e) => {
      const postId = e.target.getAttribute("postId");
      deleteDocFirebase(postId).then(() => {
        hideModal();
      }).catch((error2) => {
        console.error("Error al eliminar el documento:", error2);
      });
    });
...
const recovery = (navigateTo2) => {
  ...
  forgotButton.textContent = "recuperar contrase\xF1a";
  ...
  resetPassword.addEventListener("click", async () => {
    ...
    if (email !== "") {
      try {
        ...
        emailError.style.display = "block";
        emailError.textContent = "\xA1El correo electr\xF3nico se ha restablecido correctamente!";
      ...
      emailError.style.display = "block";
      emailError.textContent = "Debe ingresar un email v\xE1lido";
    }
  });
...
const routes = [
  { path: "/DEV006-social-network/", component: login },
...
const defaultRoute = "/DEV006-social-network/";
...
const navigateTo = (hash) => {
  ...
  if (route && route.component) {
    ...
    navigateTo("/DEV006-social-network/");
  }
};
window.onpopstate = () => {
